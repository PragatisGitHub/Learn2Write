...
def load_model(Choice):
    ...
    if (Choice == "Alpha"):
        K.clear_session()
        model = keras.models.load_model("emnist/eminst_letters_cnn_model.h5")
    elif (Choice == "Num"):  
        K.clear_session() 
        model = keras.models.load_model("emnist/eminst_digits_cnn_model.h5")   
...
def prepare_image(img):
    ...
    # Convert the image to 4 dimensional array
...
def Numbers():
    ...
    with graph.as_default():
        ...
        predicted_digit = model.predict_classes(image_array)[0]
        # pred_letter = amap[predicted_digit+1]
        data["prediction"] = int(predicted_digit)+1
        if str(predicted_digit+1) ==Symbol:
            data["result"] = True
        else:
            data["result"] = False
        # indicate that the request was a success
    ...
    for name in shell_coll.find({},{"name":1, "password":1}):
        if userID == name["name"] and password == name["password"]:
            ...
            x = shell_coll.insert_one(new_record)
            break
    # counter for true and false for current user
    trues=0
    falses=0
